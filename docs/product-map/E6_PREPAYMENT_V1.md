...
1. **预存充值**：收费员在收费台给客户充 200 → Ledger `+TOP_UP 200`，开收据，进当日日结。
2. **账单自动抵扣**：账单 POSTED 130，预存余额 100 → 自动 Allocation 抵扣 100 → 账单 PARTIAL_PAID 剩 30，余额 0。余额充足则全额抵扣 → PAID。
3. **多账单顺序**：余额 150，欠费账单 80（老）+ 60（次新）+ 50（新 POSTED）→ 依次抵扣 80、60，剩 10 抵新单 → 新单 PARTIAL_PAID 剩 40。
4. **抵扣解释**：账单详情/收据显示"预存抵扣 ¥100.00（流水号 …）"，客户能看懂钱去哪了。
5. **当日冲正**：收费员充错金额（当日未日结）→ 本人发起 REVERSAL 负向流水，原充值保留。
6. **跨日冲正/退款**：已日结的充值错误或客户主动退款 → 管理员/主管发起 REVERSAL/REFUND 负向流水。
7. **余额查询**：收费台搜索客户即见预存余额；360°/结算户页可查看全部流水。
8. **日结**：当日 TOP_UP 计入收费员日结收入；REVERSAL/REFUND 计入发生日（冲正历史日不改写）。
...
4. 自动抵扣时机：**Bill → POSTED 事务内**。账单过账即检查该结算户余额并按"最老 POSTED 欠费 → 次新 → 当前账单"顺序分配，**复用既有 Allocation**，不新建销账路径。
5. 允许部分抵扣；抵扣后账单状态按既有规则走 PARTIAL_PAID/PAID。
6. 手工收费与自动抵扣同源：收费员收现金抵扣账单 = Allocation(payment)；系统自动抵扣 = Allocation(prepaymentApply)。两种 Allocation 共享 outstanding 计算。
7. REVERSAL/REFUND 必须指向原流水并新增负向 entry；当日未日结冲正由原收费员执行，跨日冲正与客户退款需管理员/主管。
8. TOP_UP 必须开收据；APPLY 的抵扣在账单收据/详情中可解释；REFUND/REVERSAL 进流水与日结口径。
9. 幂等：充值、抵扣、冲正均要求幂等键；同一账单重复过账/重试不得产生重复 APPLY。
10. 预存余额为**正数语义**；不允许透支抵扣（抵扣上限=余额）。REFUND 不得使余额为负（退款不得超过当前余额）。
11. 监控表/不计费户不产生账单，自然不发生 APPLY；其余账务行为不特殊。
...
| 操作 | 权限 |
|---|---|
| 预存充值（TOP_UP） | 收费员（payment:write，既有） |
| 查看余额/流水 | 收费员、营业员、管理员（payment:read） |
| 系统自动抵扣（APPLY） | SYSTEM（事务内，不走用户权限） |
| 当日未日结充值冲正（REVERSAL） | 原收费员（payment:write + 同日约束） |
| 已日结冲正（REVERSAL） | 管理员/主管（payment:reverse，新权限或复用红冲权限——设计阶段定） |
| 客户主动退款（REFUND） | 管理员/主管 |
| 删除流水 | 永远禁止（无权限可授予） |
