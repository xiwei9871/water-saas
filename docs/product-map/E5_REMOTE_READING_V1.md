...
11. **晚到 REMOTE × 已终审估水**：历史期间已有 FINAL estimated settlement 时，晚到 REMOTE 经 QC PASSED 后成为 trusted reading，**复用既有 Reconciliation/Adjustment**（E4 估水恢复体系）补差——绝不原地修改 FINAL Settlement / POSTED Bill。Remote V1 不另造远传补差路径。
...
- 晚到 REMOTE 撞上已 FINAL 估水结算：QC PASSED 后成为 trusted reading，走既有 Reconciliation/Adjustment 补差，FINAL/POSTED 不被原地修改。
- 同 `externalEventKey` 不同 payload：EVENT_KEY_CONFLICT，原事件不被覆盖，不产生读数。
