...
4. **设备绑定维护**：管理员把厂商设备标识绑定到 Meter/Installation；换表后改绑，历史事件归属不变。
...
3. Raw Event **append-only**：无 update/delete 路径，纠正通过新事件或读数 supersede。
4. 幂等键：`(tenant, source, vendorDeviceKey, collectedAt, vendorSeq)`——同一设备+采集时间+厂商流水号视为同一事件，重复到达返回原事件。
5. 设备未绑定 → 事件状态 `UNBOUND`，进入待绑定队列；绑定后允许重放（replay）生成读数。
6. 一个 Raw Event 至多生成一条 MeterReading（`eventId` 唯一关联）；该读数仍可能被 supersede 更正，但 supersede 链写 MeterReading，不回写 Raw Event。
7. REMOTE 读数与 ACTUAL 同权进入 QC；QC 拒绝 → 该读数不进结算，等人工处理。
8. 同一 planItem 已有人工 ACTUAL 且 QC PASSED 时，远传晚到数据不覆盖——晚到事件照常落 Raw Event 留痕，但生成读数走 supersede/人工裁决，不静默改写有效事实。
9. 远传断采不改变计划项状态机；人工补抄永远是合法回退路径。
10. 设备绑定按 Installation 维度（换表=新安装事实，绑定跟随新安装），历史事件的读数归属不随改绑漂移。
...
| RemoteDeviceBinding | ACTIVE / REBOUND | vendorDeviceKey → meter/installation 绑定；换表产生新绑定行，旧行保留 |
| RawRemoteEvent | RECEIVED → UNBOUND / CONVERTED / FAILED | append-only；payload 原文 + 解析字段 + 幂等键 + 来源 |
| MeterReading（既有） | 沿用 | `resultType='REMOTE'`，`source` 记录来源 Adapter，`sourceEventId` 回链 Raw Event |

状态流转要点：

- `RECEIVED → UNBOUND`：无有效绑定；`UNBOUND → CONVERTED`：绑定后重放成功；
- `RECEIVED → CONVERTED`：正常路径；
- `→ FAILED`：解析失败/数据非法（保留原因，允许修复后重放——重放仍走幂等键，不产生重复）；
- FAILED/UNBOUND 事件可重放，重放是同一事件的再处理，不是新事件。

## UI Entry

- **抄表 → 远传接入**：Source 列表（类型/厂商/状态/最近采集时间/成功率）、绑定管理、事件流水（按状态过滤：全部/待绑定/失败/已转换）。
...
- 含未绑定设备的导入：该户事件 UNBOUND，其余正常转换；绑定后重放生成读数。
